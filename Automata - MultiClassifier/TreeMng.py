...
def sift(ORC, node, s):
    ...
    while len(cur.children) != 0:
        d = cur.str
        sd = add_strings(s, d)
        result = ORC.member_query(key2word(sd))
...
def update_tree(ORC, root, counter, hyp, g):
    s = []
    s_t = []
    pre_strs = get_all_prefix(counter)
    j = -1
    i = -1
    for pre in pre_strs:
        i += 1
        s_i = sift(ORC, root, pre).str
        s_t_i = hyp.predict(key2word(pre), predict_state=True)
        s.append(s_i)
        s_t.append(s_t_i)
        if s_i != s_t_i:
            j = i
    ...
    dist_d = add_strings(gamma_j, d)
    min_pre = pre_strs[j-1]
    s_j_m1 = s[j-1]

    is_found, node_to_replace = find_in_tree(s_j_m1, root)
    new_node = replace_node(ORC, node_to_replace, min_pre, s_j_m1, dist_d, g)
...
def replace_node(ORC, node, str_new, str_old, d, g):
    ...
    class_new = ORC.member_query(key2word(add_strings(str_new, d)))
    class_old = ORC.member_query(key2word(add_strings(str_old, d)))
...
def find_dist(s1, s2, start):
    ...
    for key in cur.children.keys():
        ...
        if is_1_in_key_kid: found_1 = True
        is_2_in_key_kid, drop = find_in_tree(s2, cur.children[key])
        if is_2_in_key_kid: found_2 = True
